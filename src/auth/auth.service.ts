...
export class AuthService {
    ...
    async createRefreshToken(userId: string): Promise<RefreshToken> {
        ...
        expiresAt.setDate(expiresAt.getDate() + parseInt(
            this.configService.get('REFRESH_TOKEN_EXPIRATION', '7')
        ));
    ...
    async refreshAccessToken(refreshToken: string): Promise<{ access_token: string; refresh_token: string }> {
        ...
        const tokenRecord = await this.usersService.findRefreshTokenByToken(refreshToken);
