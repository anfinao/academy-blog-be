import { Controller, Post, Body, Param, Delete, HttpCode, HttpStatus, UseGuards } from '@nestjs/common';
...
export class UsersController {
    ...
    @ApiParam({ name: 'userId', description: 'ID пользователя', example: 'uuid-123' })
    @ApiResponse({ status: 200, description: 'Пользователь успешно заблокирован' })
    ...
    @ApiParam({ name: 'userId', description: 'ID пользователя', example: 'uuid-123' })
    @ApiResponse({ status: 200, description: 'Пользователь успешно разблокирован' })
    @ApiResponse({ status: 404, description: 'Пользователь не найден' })
